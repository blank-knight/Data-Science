...
import math
import argparse
import data_tools_attention
TRAIN_DIR = './attention_model/checkpoint/'
BATCH_SIZE = 200
MAX_STEPS = 20000
...
def inference_graph(input, input_size, keep_prob):
    ...
    with tf.name_scope('attention'):
        attention_size = 50  # 根据需要调整这个值
        print("++++++++++++")
        print(pool2.shape)
        print("++++++++++++")
        attention_output = attention_layer(pool2, attention_size)
    # 全连接层
    with tf.name_scope('fc'):
        weights = tf.Variable(tf.truncated_normal([attention_size, fc_size], stddev=0.1), name='weights')
        biases = tf.Variable(tf.constant(0.1, shape=[fc_size]), name='biases')
        # 将 attention_output 转换为 [?,40] 形状
        attention_output_flat = tf.squeeze(attention_output, axis=1)
        print(attention_output_flat.get_shape().as_list())  # 添加这行代码
        fc = tf.nn.relu(tf.matmul(attention_output_flat, weights) + biases)
    # Dropout layer
...
def attention_layer(inputs, attention_size):
    # inputs形状：[batch_size, time_steps, input_units]
    ...
    # 对每个时间步应用全连接层并使用非线性激活函数
    print("++++++++++++++++++++++")
    print(inputs.shape)
    print("++++++++++++++++++++++")
    print(W_omega.shape)
    print("++++++++++++++++++++++")
    v = tf.tanh(tf.tensordot(inputs, tf.transpose(W_omega), axes=1) + b_omega)

    # 对于每个时间步的输出 y = v * u
    vu = tf.tensordot(v, u_omega, axes=1)  # 形状 (batch_size, time_steps)
    alphas = tf.nn.softmax(vu)  # 形状 (batch_size, time_steps, 1)
...
def main(_):
    ...
    data = data_tools_attention.nlos_classification_dataset(r'\dataset', split_factor = 0.6, scaling = False)
    ...
    with tf.Session(graph = graph) as sess:
        ...
        write_metrics_to_csv('./attention_metrics.csv', metrics)
